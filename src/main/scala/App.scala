...
//
      ...
      var dataset = credits.join(movies)
        ...
        => Movie(id, title, parseList(cast), getCrew(crew), budget.toInt, parseList(genres), parseList(keywords),
           parseList(productC), popularity.toDouble, voteAverage.toDouble)}).persist()

      println(dataset.collect.size)
      val test = dataset.sample(false, 0.01).collect().toList
      dataset = dataset.filter(x => !test.contains(x))
      println(test.length, dataset.collect.size)
      test.map(x => (x.title, x.voteAverage, KNN(x, dataset, n)))
        .map({case (title, real, pred) => (title, 100*Math.abs((real - pred)/real))}).foreach(println)
   }

   def KNN(inputMovie: Movie, data: RDD[Movie], n: Int): Double = {
      val rating = data.map(movie => (movie, euclidean_distance(movie, inputMovie)))
        .sortBy(movie => movie._2).take(n).map({case (movie, dist) => movie.voteAverage})
        .aggregate((0.0, 0.0))((x, y) => (x._1 + y, x._2+ 1), (x, y) => (x._1 + y._1, x._2 + y._2))
      if(rating._2 == 0){
         return 0.0
      }
      return rating._1/rating._2
   }
   ...
   def getCastDifference(movieA: Movie, movieB: Movie): Double = {
      ...
      movieA.cast.foreach(a => {
         ...
         if(i == -1){
            ...
            x += 100/Math.max(Math.min(movieA.cast.size, movieB.cast.size), 1)
         }
      })
      return 100.0/x
   }
      ...
      for(a <- movieA.genres){
         for(b <- movieB.genres){
            if(a.contains(b) || b.contains(a)){
               x += 100/Math.max(Math.min(movieA.genres.size, movieB.genres.size), 1)
            }
         }
      }
      return 100.0/x
