...
object App {
   ...
   case class Movie(id: String, title: String, cast:List[String], crew: List[String], budget:Int, genres: String, keyWords: String,
                    productionCompanies: String, popularity: Double, voteAverage:Double)
...
//      test.map(x => (x._2._1._1, calculateRating(x, dataset)))
   ...
   def getCast(cast: String): List[String] = {
      val parsed = JSON.parseFull(cast).get.asInstanceOf[List[Map[String, String]]]
...
//   def calculateRating(movie:Movie, data: RDD[Movie]): Double = {
//      data.take(10).map(x => (x , getCastDifference(movie, x))).foreach(println)
//      return 0
//   }
   //a z
   //a b z ->
